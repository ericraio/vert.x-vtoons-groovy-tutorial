...
(function DemoViewModel() {
  ...
  that.submitOrder = function() {
    ...
    // ordermsg json object
    var orderMsg = {
      // sessionID from user when we logged in
      sessionID: that.sessionID(),
      // save the action
      action: "save",
      // collection is set to orders
      collection: "orders",
      document: {
    ...
    // send a json message to vertx.mongopersistor with the order message, then the function is the call back from the server
    eb.send('vertx.mongopersistor', orderMsg, function(reply) {
      // callback reply evaluate status
      if (reply.status === 'ok') {
        ...
        // output a console.error if the order was not a ok status
